...
AVst::AVst (audioMasterCallback audioMaster)
	: AudioEffectX (audioMaster, 1, 7)	// 1 program, 6 parameters
{
    ...
    modeMakeUp = 1; // =ON (due to detector madness)
...
void AVst::getParameterName (long index, char *label)
{
    switch (index)
	{
		...
		case  6 : strcpy (label, "Gain Reduction");        break;
	}
}
...
void AVst::getParameterDisplay (long index, char *text)
{
	switch (index)
	{
        ...
        break;
		case  5 : float2string (amp2DB(outVolume), text);        break;
		case  6 : float2string (amp2DB(GR), text);        break;
	}
}
...
{
	strcpy (name, "Digital Versatile Compressor LITE /w Meter");
	return true;
}
...
{
	strcpy (text, "Digital Versatile Compressor LITE /w Meter");
	return true;
}
