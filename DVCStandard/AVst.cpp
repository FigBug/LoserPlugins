...
AVst::AVst (audioMasterCallback audioMaster)
	: AudioEffectX (audioMaster, 1, 16)	// 1 program, 15 parameters
{
...
void AVst::getParameterDisplay (long index, char *text)
{
	switch (index)
	{
        ...
        break;
		case 14 : dB2string (outVolume, text);        break;
		case 15 : dB2string (dryMix, text);           break;
	}
}
...
{
	strcpy (name, "Digital Versatile Compressor STANDARD");
	return true;
}
...
{
	strcpy (text, "Digital Versatile Compressor STANDARD");
	return true;
}
