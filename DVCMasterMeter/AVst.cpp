...
AVst::AVst (audioMasterCallback audioMaster)
	: AudioEffectX (audioMaster, 1, 21)	// 1 program, 20 parameters
{
...
void AVst::getParameterName (long index, char *label)
{
    switch (index)
	{
		...
		case 20 : strcpy (label, "Gain Reduction");        break;
	}
}
...
void AVst::getParameterDisplay (long index, char *text)
{
	switch (index)
	{
        ...
        break;
		case 17 : float2string ( amp2DB(bypassMix), text);        break;
		case 18 : float2string ( amp2DB(outVolume), text);        break;
		case 19 : float2string ( amp2DB(dryMix), text);           break;
		case 20 : float2string ( amp2DB(GR), text);           break;
	}
}
...
{
	strcpy (name, "Digital Versatile Compressor MASTER /w Meter");
	return true;
}
...
{
	strcpy (text, "Digital Versatile Compressor MASTER /w Meter");
	return true;
}
