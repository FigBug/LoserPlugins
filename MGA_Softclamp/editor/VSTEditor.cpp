...
void percent2Display (float value, char* string);
void percent2Display (float value, char* string)
{
    sprintf ( string, "%.0f%%", 100*value );
}
...
//    threshControl->setTag( kGain );
	...
	size (kKneeX, kKneeY,
          kKneeX + kControlWidth, kKneeY + kControlHeight);
    kneeControl = new CParamControl (size, this, kKnee, 0);
    kneeControl->setTransparency(true); 
    kneeControl->setFontColor (kWhiteCColor);
    kneeControl->setFont (kNormalFontSmaller); 
    kneeControl->setHoriAlign(kRightText);
	kneeControl->setValue (effect->getParameter (kKnee));
	kneeControl->setDefaultValue (0.5f);
	kneeControl->setRange(10);
	kneeControl->setFineRange(100);
    kneeControl->setStringConvert( percent2Display );
//    threshControl->setTag( kGain );
    lFrame->addView( kneeControl );
...
void VSTEditor::setParameter (VstInt32 index, float value)
{
	...
	switch (index)
	{
		...
		case kKnee :
            if(kneeControl)
	           kneeControl->setValue ( effect->getParameter (index));
			break;
	}
...
void VSTEditor::idle ()
{
    ...
    GRDisplayL->setValue ( effect->getVU( kGRPeakL ) );
    GRDisplayR->setValue ( effect->getVU( kGRPeakR ) );
    GRMeterL->setValue (  (5  +  max( (log( effect->getVU( kGRL ) ) * 8.685889638) , -5 ))/5   );
    GRMeterR->setValue (  (5  +  max( (log( effect->getVU( kGRR ) ) * 8.685889638) , -5 ))/5   );
