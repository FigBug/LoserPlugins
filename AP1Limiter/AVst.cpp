	...
	: AudioEffectX (audioMaster, 1, 6)	// 1 program, 15 parameters
{
    ...
    attackStartSpls = static_cast<unsigned long>( 0.0015 * getSampleRate() ); // 0 ms no lookAhead, no delayed attack
    lookAheadSize = static_cast<unsigned long>( 0.0015 * getSampleRate() );
    setInitialDelay(lookAheadSize);
	lookAheadBufferRms = new float[lookAheadSize];
	lookAheadBufferIn1 = new float[lookAheadSize];
	lookAheadBufferIn2 = new float[lookAheadSize];

    attackSpls = 0.001 * getSampleRate(); // =2 ms
    attack = dB2Amp( -30 / max( attackSpls , 0) );
...
{
    lookAheadBufferRms = (float*) realloc(lookAheadBufferRms, lookAheadSize * sizeof(float));
    lookAheadBufferIn1 = (float*) realloc(lookAheadBufferIn1, lookAheadSize * sizeof(float));
    lookAheadBufferIn2 = (float*) realloc(lookAheadBufferIn2, lookAheadSize * sizeof(float));
}
...
void AVst::suspend()
{
    ...
    lookAheadBufPos = 0;
    if (lookAheadSize != static_cast<unsigned long>( 0.0015 * getSampleRate() ))
    { 
        lookAheadSize = static_cast<unsigned long>( 0.0015 * getSampleRate() );
    ...
    attackSpls = 0.001 * getSampleRate(); // =1 ms
    attack = dB2Amp( -30 / max( attackSpls , 0) );
...
{
	strcpy (name, "Advanced Peak-1 Limiter");
	return true;
}
...
{
	strcpy (text, "Advanced Peak-1 Limiter");
	return true;
}
...
// ENVELOPE FOLLOWER: smooth the detector signal, so it represents the envelope curve of the incoming waveform
    seekMaxSpls = sqrt( (tmpEnvLP = a0EnvLP*rms - b1EnvLP*tmpEnvLP + cDcAdd_) - cDcAdd_ );
